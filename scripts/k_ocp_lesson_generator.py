#!/usr/bin/env python3
"""K-OCP (Red Hat OpenShift) lesson generator.

Mirrors scripts/k_gke_lesson_generator.py for the K-OCP curriculum.
Universe: K-Foundry — Red Hat enterprise factory; 13 bays mapping O1-O13.
Pin prefix `ko-bay` (K-OCP-namespaced; the K-Foundry universe name is for
...
# ---------------------------------------------------------------------------
# K-Foundry atlas — 13 bays / facilities of the Red Hat enterprise factory
# ---------------------------------------------------------------------------
...
TOTAL_LESSONS_KOCP = 13


def _strip_index_kocp(num: str) -> int | None:
    try:
        return int(num) - 1
    except ValueError:
        return None
...
def _render_bay(bay: tuple, active_id: str) -> str:
    slug, num, x, y, label, sub, special = bay
...
def _render_kfoundry_map(active_bay: str, lesson_num: str, bay_label: str) -> str:
    bays_svg = "\n    ".join(_render_bay(b, active_bay) for b in KFOUNDRY_BAYS)
    active_idx = _strip_index_kocp(lesson_num)
    strip_parts = []
    for i in range(TOTAL_LESSONS_KOCP):
        cls = "ktown-strip-pin"
        if i == 0:
    ...
    strip_html = "\n    ".join(strip_parts)
    aria_label = f"Module O{int(lesson_num)}, {bay_label}."
    return f"""<div class="ktown-map-wrap">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 420" class="ktown-map" role="img" aria-label="K-Foundry map: O{int(lesson_num)}">
    <title>K-Foundry map · O{int(lesson_num)}</title>
    <desc>Stylised Red Hat OpenShift enterprise factory. {bay_label} highlighted.</desc>
    <defs>
      <linearGradient id="ko-floor" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#FBF1D6" stop-opacity="0.7"/><stop offset="100%" stop-color="#E8DDC8" stop-opacity="0.55"/></linearGradient>
      <linearGradient id="ko-fire" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#A04832" stop-opacity="0.16"/><stop offset="100%" stop-color="#3F4A5E" stop-opacity="0.30"/></linearGradient>
    </defs>
    <rect x="20" y="20" width="760" height="380" rx="14" fill="url(#ko-floor)" stroke="#A04832" stroke-width="1"/>
    <text x="400" y="38" text-anchor="middle" font-family="ui-rounded,sans-serif" font-size="10" font-weight="700" letter-spacing="3" fill="#9D9389">K · FOUNDRY · RED HAT OPENSHIFT</text>
    <line x1="40" y1="130" x2="780" y2="130" stroke="#5F5E5A" stroke-width="0.7" stroke-dasharray="2,4" opacity="0.4"/>
    <line x1="40" y1="240" x2="780" y2="240" stroke="#5F5E5A" stroke-width="0.7" stroke-dasharray="2,4" opacity="0.4"/>
    <line x1="40" y1="350" x2="780" y2="350" stroke="#5F5E5A" stroke-width="0.7" stroke-dasharray="2,4" opacity="0.3"/>
    <text x="48" y="120" font-family="sans-serif" font-size="8" font-style="italic" fill="#9D9389">welcome &amp; foundation bays</text>
    <text x="48" y="232" font-family="sans-serif" font-size="8" font-style="italic" fill="#9D9389">production &amp; ops bays</text>
    <text x="48" y="342" font-family="sans-serif" font-size="8" font-style="italic" fill="#9D9389">specialty &amp; multi-foundry bays</text>
    {bays_svg}
  </svg>
  <ol class="ktown-strip" aria-hidden="true">
    {strip_html}
  </ol>
  <p class="ktown-strip-label">\U0001F3ED <strong>{bay_label}</strong> <span>· O{int(lesson_num)} of {TOTAL_LESSONS_KOCP}</span><span class="visually-hidden">{aria_label}</span></p>
</div>"""


def _render_concept_rail_kocp(current_num: str) -> str:
    items = []
    found = False
    for num, label in K_OCP_RAIL:
        if num == current_num:
    ...
    return f"""<aside class="concept-rail" aria-label="K-OCP module journey">
  <span class="concept-rail-title">K-OCP modules</span>
{chr(10).join(items)}
</aside>"""
...
def render_lesson_kocp(spec: LessonSpec) -> str:
    css = BASE_CSS + (spec.extra_css or "")
    map_html = _render_kfoundry_map(spec.district_pin, spec.num, spec.district_label)
    rail_html = _render_concept_rail_kocp(spec.num)
    ...
    <h2>The story for this bay</h2>
{spec.analogy_intro_html}
    <div class="translation-legend">
      <table>
        <thead><tr><th>In the story…</th><th>…in OpenShift / Red Hat</th></tr></thead>
        <tbody>
{legend_rows}
        </tbody>
      </table>
    ...
    <h2>How OpenShift teams do this</h2>
    <div class="scenarios">
{scenarios_html}
    </div>
  </section>"""
...
{glossary_html}
    ...
    <span class="recap-badge">✓ Module O{int(spec.num)} complete</span>
    <p>{spec.recap_lead}</p>
...
<header class="topbar">
  <div class="topbar-inner">
    <a href="#" class="brand"><span class="brand-mark">SG</span><span>Suvis Guru</span><span class="course"> · K-OCP · Module O{int(spec.num)}</span></a>
    <span class="progress-pill">O{int(spec.num)} · {spec.title_short}</span>
    <button class="theme-toggle" id="theme-toggle" type="button">\U0001F319 Dark</button>
  </div>
</header>

<p class="district-line">\U0001F3ED K-Foundry bay: <strong>{spec.district_label}</strong>.</p>
...
<footer>Suvis Guru · K-OCP · Module O{int(spec.num)} of {TOTAL_LESSONS_KOCP} · grounded in docs.openshift.com</footer>
...
def _maybe_attach_animation_kocp(spec: LessonSpec, spec_path: str) -> None:
    ...
    if spec_dir not in sys.path:
        sys.path.insert(0, spec_dir)
    spec_obj = importlib.util.spec_from_file_location("animations_kocp", anim_path)
    mod = importlib.util.module_from_spec(spec_obj)
    spec_obj.loader.exec_module(mod)
...
def _run_audits_kocp() -> int:
    import subprocess
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "audit_lessons_kocp.py")
    if not os.path.exists(path):
        return 0
    result = subprocess.run([sys.executable, path], capture_output=True, text=True)
    if result.returncode != 0:
        print("\n>>> audit_lessons_kocp.py FAILED <<<")
        print(result.stdout)
        if result.stderr:
            print(result.stderr, file=sys.stderr)
        return 1
    print(f"audit: audit_lessons_kocp.py ✓ ({(result.stdout.splitlines() or [''])[0]})")
    return 0


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("specs", nargs="+", help="K-OCP spec modules")
    ...
    for spec_path in args.specs:
        ...
        _maybe_attach_animation_kocp(spec, spec_path)
        html = render_lesson_kocp(spec)
        out_path = os.path.join(args.out, f"preview-kubernetes-ocp-lesson-{spec.num}.html")

#!/usr/bin/env python3
"""Build a single-file combined view of all K-EKS modules.

Mirror of scripts/build_combined_course_kvan.py, targeting K-EKS.
Output: preview-kubernetes-eks-course-all.html at the repo root.
"""
...
LESSONS = [
    ("01", "preview-kubernetes-eks-lesson-01.html", "E1 · EKS Architecture and Shared Responsibility"),
    ("02", "preview-kubernetes-eks-lesson-02.html", "E2 · EKS Auto Mode"),
    ("03", "preview-kubernetes-eks-lesson-03.html", "E3 · AWS Networking for EKS"),
    ("04", "preview-kubernetes-eks-lesson-04.html", "E4 · Identity and Access"),
    ("05", "preview-kubernetes-eks-lesson-05.html", "E5 · EKS Storage"),
    ("06", "preview-kubernetes-eks-lesson-06.html", "E6 · Compute and Autoscaling"),
    ("07", "preview-kubernetes-eks-lesson-07.html", "E7 · EKS Security"),
    ("08", "preview-kubernetes-eks-lesson-08.html", "E8 · EKS Observability"),
    ("09", "preview-kubernetes-eks-lesson-09.html", "E9 · Upgrades and Operations"),
    ("10", "preview-kubernetes-eks-lesson-10.html", "E10 · EKS Troubleshooting"),
    ("11", "preview-kubernetes-eks-lesson-11.html", "E11 · Capstone — Multi-AZ Auto Mode Tower"),
]
...
def main() -> None:
    ...
    out.append(f"<title>K-EKS · The Whole Course (all {len(LESSONS)} modules)</title>")
    out.append("<style>")
    for b in bundles:
        out.append(f"\n/* ============ {b['fname']} ============ */")
        out.append(b["style"])
...
""")
    ...
    out.append('  <a href="#course-toc" class="brand"><span class="brand-mark">SG</span><span>Suvis Guru</span><span class="brand-meta"> · K-EKS · the whole course</span></a>')
    ...
    out.append('<aside class="course-sidebar" aria-label="K-EKS module navigation">')
    out.append(f'  <h3>The {len(LESSONS)} modules</h3>')
    out.append('  <ol>')
    for b in bundles:
        out.append(f'    <li><a href="#lesson-{b["num"]}"><span class="num">E{int(b["num"])}</span><span>{b["title"]}</span></a></li>')
    out.append('  </ol>')
    out.append('</aside>')

    out.append('<section class="course-toc" id="course-toc">')
    out.append("  <h2>K-EKS — Amazon EKS, AWS-managed Kubernetes</h2>")
    out.append(f"  <p class=\"toc-sub\">All {len(LESSONS)} K-EKS modules concatenated. Prereq: K-COM curriculum + AWS basics (VPC, IAM, EC2, S3, CloudWatch).</p>")
...
""")
    ...
    output_path = os.path.join(ROOT, "preview-kubernetes-eks-course-all.html")
    text = "\n".join(out)

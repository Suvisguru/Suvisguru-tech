#!/usr/bin/env python3
"""Build a single-file combined view of all K-VAN modules.

Mirror of scripts/build_combined_course.py, targeting K-VAN.
Output: preview-kubernetes-vanilla-course-all.html at the repo root.
"""
...
LESSONS = [
    ("01", "preview-kubernetes-vanilla-lesson-01.html", "V1 · Production Architecture Design"),
    ("02", "preview-kubernetes-vanilla-lesson-02.html", "V2 · OS and Node Preparation"),
    ("03", "preview-kubernetes-vanilla-lesson-03.html", "V3 · kubeadm Cluster Bootstrap"),
    ("04", "preview-kubernetes-vanilla-lesson-04.html", "V4 · CNI Installation and Networking"),
    ("05", "preview-kubernetes-vanilla-lesson-05.html", "V5 · Core Add-ons"),
    ("06", "preview-kubernetes-vanilla-lesson-06.html", "V6 · Cluster Configuration"),
    ("07", "preview-kubernetes-vanilla-lesson-07.html", "V7 · etcd Production"),
    ("08", "preview-kubernetes-vanilla-lesson-08.html", "V8 · Upgrades and Patching"),
    ("09", "preview-kubernetes-vanilla-lesson-09.html", "V9 · Security Hardening"),
    ("10", "preview-kubernetes-vanilla-lesson-10.html", "V10 · Advanced Troubleshooting"),
    ("11", "preview-kubernetes-vanilla-lesson-11.html", "V11 · Capstone Homestead"),
]
...
def main() -> None:
    ...
    out.append(f"<title>K-VAN · The Whole Course (all {len(LESSONS)} modules)</title>")
    out.append("<style>")
    for b in bundles:
        out.append(f"\n/* ============ {b['fname']} ============ */")
        out.append(b["style"])
...
""")
    ...
    out.append('  <a href="#course-toc" class="brand"><span class="brand-mark">SG</span><span>Suvis Guru</span><span class="brand-meta"> · K-VAN · the whole course</span></a>')
    ...
    out.append('<aside class="course-sidebar" aria-label="K-VAN module navigation">')
    out.append(f'  <h3>The {len(LESSONS)} modules</h3>')
    out.append('  <ol>')
    for b in bundles:
        out.append(f'    <li><a href="#lesson-{b["num"]}"><span class="num">V{int(b["num"])}</span><span>{b["title"]}</span></a></li>')
    out.append('  </ol>')
    out.append('</aside>')

    out.append('<section class="course-toc" id="course-toc">')
    out.append("  <h2>K-VAN — vanilla self-managed Kubernetes</h2>")
    out.append(f"  <p class=\"toc-sub\">All {len(LESSONS)} K-VAN modules concatenated. Prereq: K-COM curriculum complete.</p>")
...
""")
    ...
    output_path = os.path.join(ROOT, "preview-kubernetes-vanilla-course-all.html")
    text = "\n".join(out)

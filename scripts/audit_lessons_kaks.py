#!/usr/bin/env python3
"""K-AKS cross-lesson audit — mechanical + content (mirror of K-EKS audit)."""
...
LESSON_FILES = sorted(
    ...
    if f.startswith("preview-kubernetes-aks-lesson-") and f.endswith(".html")
)

EXPECTED_ACTIVE_WING = {f"{i:02d}": f"kc-wing{i:02d}" for i in range(1, 12)}
EXPECTED_DOTS = 11
TOTAL_LESSONS = 11
...
def report(category, lesson, msg):
    issues[category].append(f"A{int(lesson)}: {msg}")


for filename in LESSON_FILES:
    m = re.match(r"preview-kubernetes-aks-lesson-(\d+)\.html", filename)
    if not m:
        continue
    num = m.group(1)
    ...
    wing_ids = re.findall(r'id="(kc-wing\d+)"', content)
    if len(wing_ids) != 11:
        report("KCAMPUS", num, f"expected 11 K-Campus wings, found {len(wing_ids)}")
    active = re.findall(r'<g class="pin[^"]*active[^"]*" id="(kc-wing\d+)"', content)
    expected = EXPECTED_ACTIVE_WING.get(num)
    if expected and (not active or active[0] != expected):
        report("KCAMPUS", num, f"active wing = {active[0] if active else 'NONE'}, expected {expected}")

    strip_pins = re.findall(r'<li class="ktown-strip-pin[^"]*"', content)
    if len(strip_pins) != EXPECTED_DOTS:
        report("KCAMPUS", num, f"expected {EXPECTED_DOTS} strip dots, found {len(strip_pins)}")

    of_matches = re.findall(r'A\d+\s+of\s+(\d+)', content)
    for m_str in of_matches:
        if int(m_str) != TOTAL_LESSONS:
            report("FOOTER", num, f'"A… of {m_str}" — expected of {TOTAL_LESSONS}')
...
print(f"Audited {len(LESSON_FILES)} K-AKS lesson files.")

#!/usr/bin/env python3
"""Build a single-file combined view of all K-OCP modules.

Mirror of scripts/build_combined_course_kgke.py, targeting K-OCP (13 modules).
"""
...
LESSONS = [
    ("01", "preview-kubernetes-ocp-lesson-01.html", "O1 · OpenShift Architecture"),
    ("02", "preview-kubernetes-ocp-lesson-02.html", "O2 · Installation Models"),
    ("03", "preview-kubernetes-ocp-lesson-03.html", "O3 · OpenShift Networking"),
    ("04", "preview-kubernetes-ocp-lesson-04.html", "O4 · OpenShift Security"),
    ("05", "preview-kubernetes-ocp-lesson-05.html", "O5 · Operators and OLM"),
    ("06", "preview-kubernetes-ocp-lesson-06.html", "O6 · Workloads + DevEx"),
    ("07", "preview-kubernetes-ocp-lesson-07.html", "O7 · OpenShift Storage"),
    ("08", "preview-kubernetes-ocp-lesson-08.html", "O8 · OpenShift Operations"),
    ("09", "preview-kubernetes-ocp-lesson-09.html", "O9 · Virtualization + AI + Edge"),
    ("10", "preview-kubernetes-ocp-lesson-10.html", "O10 · Multi-Cluster (RHACM)"),
    ("11", "preview-kubernetes-ocp-lesson-11.html", "O11 · OpenShift Observability"),
    ("12", "preview-kubernetes-ocp-lesson-12.html", "O12 · OpenShift Troubleshooting"),
    ("13", "preview-kubernetes-ocp-lesson-13.html", "O13 · Capstone — Reference Foundry"),
]
...
def main():
    ...
    out = ["<!DOCTYPE html>", '<html lang="en">', "<head>", '<meta charset="UTF-8">',
        ...
        f"<title>K-OCP · The Whole Course (all {len(LESSONS)} modules)</title>", "<style>"]
    for b in bundles:
        out.append(f"\n/* ============ {b['fname']} ============ */")
        out.append(b["style"])
    out.append("""
...
""")
    ...
    out.append('  <a href="#course-toc" class="brand"><span class="brand-mark">SG</span><span>Suvis Guru</span><span class="brand-meta"> · K-OCP · the whole course</span></a>')
    ...
    out.append("</header>")
    out.append('<aside class="course-sidebar" aria-label="K-OCP module navigation">')
    out.append(f'  <h3>The {len(LESSONS)} modules</h3>')
    out.append('  <ol>')
    for b in bundles:
        out.append(f'    <li><a href="#lesson-{b["num"]}"><span class="num">O{int(b["num"])}</span><span>{b["title"]}</span></a></li>')
    out.append('  </ol></aside>')
    out.append('<section class="course-toc" id="course-toc">')
    out.append("  <h2>K-OCP — Red Hat OpenShift Container Platform</h2>")
    out.append(f'  <p class="toc-sub">All {len(LESSONS)} K-OCP modules concatenated. Prereq: K-COM curriculum + Reference version OpenShift Container Platform 4.21+.</p>')
...
""")
    ...
    output_path = os.path.join(ROOT, "preview-kubernetes-ocp-course-all.html")
    text = "\n".join(out)

#!/usr/bin/env python3
"""Build a single-file combined view of all K-AKS modules.

Mirror of scripts/build_combined_course_keks.py, targeting K-AKS.
Output: preview-kubernetes-aks-course-all.html at the repo root.
"""
...
LESSONS = [
    ("01", "preview-kubernetes-aks-lesson-01.html", "A1 · AKS Architecture and Shared Responsibility"),
    ("02", "preview-kubernetes-aks-lesson-02.html", "A2 · Azure Identity and Access"),
    ("03", "preview-kubernetes-aks-lesson-03.html", "A3 · AKS Networking"),
    ("04", "preview-kubernetes-aks-lesson-04.html", "A4 · AKS Storage"),
    ("05", "preview-kubernetes-aks-lesson-05.html", "A5 · AKS Scaling"),
    ("06", "preview-kubernetes-aks-lesson-06.html", "A6 · AKS Security"),
    ("07", "preview-kubernetes-aks-lesson-07.html", "A7 · AKS Observability"),
    ("08", "preview-kubernetes-aks-lesson-08.html", "A8 · Add-ons and Platform Features"),
    ("09", "preview-kubernetes-aks-lesson-09.html", "A9 · Upgrades and Operations"),
    ("10", "preview-kubernetes-aks-lesson-10.html", "A10 · AKS Troubleshooting"),
    ("11", "preview-kubernetes-aks-lesson-11.html", "A11 · Capstone — Reference Campus"),
]
...
def main() -> None:
    ...
    out.append(f"<title>K-AKS · The Whole Course (all {len(LESSONS)} modules)</title>")
    out.append("<style>")
    for b in bundles:
        out.append(f"\n/* ============ {b['fname']} ============ */")
        out.append(b["style"])
...
""")
    ...
    out.append('  <a href="#course-toc" class="brand"><span class="brand-mark">SG</span><span>Suvis Guru</span><span class="brand-meta"> · K-AKS · the whole course</span></a>')
    ...
    out.append('<aside class="course-sidebar" aria-label="K-AKS module navigation">')
    out.append(f'  <h3>The {len(LESSONS)} modules</h3>')
    out.append('  <ol>')
    for b in bundles:
        out.append(f'    <li><a href="#lesson-{b["num"]}"><span class="num">A{int(b["num"])}</span><span>{b["title"]}</span></a></li>')
    out.append('  </ol>')
    out.append('</aside>')

    out.append('<section class="course-toc" id="course-toc">')
    out.append("  <h2>K-AKS — Azure AKS, Microsoft-managed Kubernetes</h2>")
    out.append(f"  <p class=\"toc-sub\">All {len(LESSONS)} K-AKS modules concatenated. Prereq: K-COM curriculum + Azure basics (Entra ID, VNet, VMSS, LB, Disks/Files, Monitor, Key Vault).</p>")
...
""")
    ...
    output_path = os.path.join(ROOT, "preview-kubernetes-aks-course-all.html")
    text = "\n".join(out)

#!/usr/bin/env python3
"""K-GKE scenario folder builder — mirror of build_scenario_folders_kaks.py for K-GKE.

For each K-GKE module G1-G10, writes the four canonical files under
courses/kubernetes/google-gke/scenarios/{nn-slug}/.
"""
...
SCENARIOS = os.path.join(ROOT, "courses/kubernetes/google-gke/scenarios")
LESSONS_DIR = os.path.join(ROOT, "scripts/lessons_kgke")
...
LESSON_META = {
    "01": ("g1-gke-architecture", "Module G1 · GKE Architecture and Modes", "G1 · GKE Architecture and Modes (Standard, Autopilot, Enterprise)"),
    "02": ("g2-versioning-and-channels", "Module G2 · Versioning and Release Channels", "G2 · GKE Versioning and Release Channels"),
    "03": ("g3-gke-networking", "Module G3 · GKE Networking", "G3 · GKE Networking (VPC-native, Dataplane V2, Gateway, multi-cluster, CSM)"),
    "04": ("g4-identity-and-security", "Module G4 · Identity and Security", "G4 · GKE Identity and Security"),
    "05": ("g5-gke-storage", "Module G5 · GKE Storage", "G5 · GKE Storage (PD, Hyperdisk + Storage Pools, Filestore, GCS FUSE, Parallelstore)"),
    "06": ("g6-scaling-and-cost", "Module G6 · Scaling and Cost", "G6 · GKE Scaling and Cost"),
    "07": ("g7-gke-observability", "Module G7 · GKE Observability", "G7 · GKE Observability (Cloud Logging + Monitoring + GMP + Grafana + Trace + Profiler + SLO)"),
    "08": ("g8-enterprise-and-ai-ml", "Module G8 · Enterprise (Fleets) and AI/ML", "G8 · GKE Enterprise (Fleets) and AI/ML on GKE"),
    "09": ("g9-troubleshooting", "Module G9 · GKE Troubleshooting", "G9 · GKE Troubleshooting (GCP-Specific)"),
    "10": ("g10-capstone", "Module G10 · Capstone Garden", "G10 · Capstone — Regional Autopilot Reference Garden with AI Inference"),
}
...
def emit_brief(num, slug, module, title, spec):
    ...
    metaphor = f"K-Garden · {spec.district_label} — {html_to_md(spec.analogy_intro_html)[:200]}"
    parts = [
        f"# Intake brief — G{int(num)}, K-GKE",
        f"# Topic: {title}",
        f"# {module}",
        "",
        "lesson:",
        ...
        "    - 'GCP basics: IAM, VPC, GCE, Cloud LB, PD/Filestore, Cloud Logging/Monitoring.'",
        f"    - 'K-GKE modules 1-{int(num) - 1 if int(num) > 1 else 0} (cumulative).'",
    ...
    parts += [
        ...
        "  modes_count: 1-2",
        f"  description: 'See preview-kubernetes-gke-lesson-{num}.html Section 6 for the live animation.'",
        "",
        ...
        "",
        "shared_artifacts:",
        f"  preview_page: {{ file: '/preview-kubernetes-gke-lesson-{num}.html' }}",
...
def emit_lesson_md(num, title, module, spec):
    out = [
        f"# K-GKE G{int(num)} — {title}",
        "",
        f"> Course: Google GKE (K-GKE, prereq: K-COM + GCP basics)",
        f"> {module}",
        f"> Companion preview: `/preview-kubernetes-gke-lesson-{num}.html`.",
        "",
        "---",
        "",
...
def emit_flashcards(spec):
    out = [f"# Flashcards — K-GKE G (count: {len(spec.flashcards)})", "", "cards:"]
...
def emit_quiz(spec):
    out = [f"# Quiz — K-GKE G (count: {len(spec.quizzes)})", "", "questions:"]
    ...
    for num, (slug, module, title) in LESSON_META.items():
        ...
        for fname, content in files.items():
            with open(os.path.join(folder, fname), "w") as f:
                f.write(content)
        print(f"  G{int(num)}: wrote {folder}")

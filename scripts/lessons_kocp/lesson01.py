...
LESSON = LessonSpec(
    ...
    architecture_svg='''<svg viewBox="0 0 760 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="OCP architecture: CVO orchestrates ~30 ClusterOperators; MCO + RHCOS handle nodes; OLM handles addon operators; OAuth + Routes for identity + ingress.">
  <rect x="10" y="10" width="740" height="260" rx="12" fill="#FBF7F0" stroke="#A04832"/>
  <text x="380" y="32" text-anchor="middle" font-size="13" font-weight="700" fill="#3F4A5E" letter-spacing="1">OCP CONTROL PLANE · CVO ORCHESTRATES ~30 CLUSTEROPERATORS</text>
  <rect x="30" y="50" width="700" height="60" rx="8" fill="#3F4A5E"/>
  <text x="380" y="70" text-anchor="middle" font-size="11" font-weight="700" fill="#FBF1D6">CVO (Cluster Version Operator)</text>
  <text x="380" y="84" text-anchor="middle" font-size="9" fill="#FBF1D6" font-style="italic">drives upgrades · reconciles ~30 ClusterOperators</text>
  <text x="380" y="100" text-anchor="middle" font-size="9" fill="#FBE8DC">authentication · console · dns · etcd · ingress · monitoring · network · storage · machine-config · ...</text>
  <rect x="30" y="120" width="220" height="55" rx="6" fill="#5A9F7A" stroke="#3F4A5E"/>
  <text x="140" y="140" text-anchor="middle" font-size="11" font-weight="700" fill="#FFFFFF">MCO + RHCOS</text>
  <text x="140" y="156" text-anchor="middle" font-size="9" fill="#FBF1D6" font-style="italic">immutable nodes · MachineConfig</text>
  <text x="140" y="168" text-anchor="middle" font-size="8" fill="#FBE8DC">per-pool ostree updates</text>
  <rect x="270" y="120" width="220" height="55" rx="6" fill="#A04832" stroke="#3F4A5E"/>
  <text x="380" y="140" text-anchor="middle" font-size="11" font-weight="700" fill="#FFFFFF">OLM</text>
  <text x="380" y="156" text-anchor="middle" font-size="9" fill="#FBF1D6" font-style="italic">CatalogSource → Subscription → CSV</text>
  <text x="380" y="168" text-anchor="middle" font-size="8" fill="#FBE8DC">addon operator lifecycle</text>
  <rect x="510" y="120" width="220" height="55" rx="6" fill="#5E4A8E" stroke="#3F4A5E"/>
  <text x="620" y="140" text-anchor="middle" font-size="11" font-weight="700" fill="#FBF1D6">OAuth + Routes</text>
  <text x="620" y="156" text-anchor="middle" font-size="9" fill="#FBF1D6" font-style="italic">identity · TLS termination</text>
  <text x="620" y="168" text-anchor="middle" font-size="8" fill="#FBE8DC">edge / passthrough / re-encrypt</text>
  <rect x="30" y="190" width="700" height="65" rx="8" fill="#FBE8DC" stroke="#A04832"/>
  <text x="380" y="210" text-anchor="middle" font-size="11" font-weight="700" fill="#A04832">RHCOS worker nodes (per MachineConfigPool)</text>
  <text x="380" y="226" text-anchor="middle" font-size="9" fill="#5A4F45">kubelet · CRI-O · OVN-K · monitoring agent · log forwarder · MCO daemon</text>
  <text x="380" y="244" text-anchor="middle" font-size="9" fill="#5A4F45" font-style="italic">8 deployment shapes: self-managed · ROSA · ARO · OSD · OKD · HyperShift · MicroShift · SNO/CRC</text>
</svg>''',
    architecture_caption='CVO orchestrates ~30 ClusterOperators that own each piece of OCP (auth, dns, etcd, ingress, etc.); MCO + RHCOS handle node lifecycle; OLM handles addon operators; OAuth + Routes handle identity + ingress. Same code across all 8 deployment shapes.',
)

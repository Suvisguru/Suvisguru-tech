#!/usr/bin/env python3
"""K-OCP cross-lesson audit — mechanical + content (mirror of K-GKE audit, 13 modules)."""
...
LESSON_FILES = sorted(
    ...
    if f.startswith("preview-kubernetes-ocp-lesson-") and f.endswith(".html")
)

EXPECTED_ACTIVE_BAY = {f"{i:02d}": f"ko-bay{i:02d}" for i in range(1, 14)}
EXPECTED_DOTS = 13
TOTAL_LESSONS = 13
...
def report(category, lesson, msg):
    issues[category].append(f"O{int(lesson)}: {msg}")


for filename in LESSON_FILES:
    m = re.match(r"preview-kubernetes-ocp-lesson-(\d+)\.html", filename)
    if not m:
        continue
    num = m.group(1)
    ...
    bay_ids = re.findall(r'id="(ko-bay\d+)"', content)
    if len(bay_ids) != TOTAL_LESSONS:
        report("KFOUNDRY", num, f"expected {TOTAL_LESSONS} K-Foundry bays, found {len(bay_ids)}")
    active = re.findall(r'<g class="pin[^"]*active[^"]*" id="(ko-bay\d+)"', content)
    expected = EXPECTED_ACTIVE_BAY.get(num)
    if expected and (not active or active[0] != expected):
        report("KFOUNDRY", num, f"active bay = {active[0] if active else 'NONE'}, expected {expected}")

    strip_pins = re.findall(r'<li class="ktown-strip-pin[^"]*"', content)
    if len(strip_pins) != EXPECTED_DOTS:
        report("KFOUNDRY", num, f"expected {EXPECTED_DOTS} strip dots, found {len(strip_pins)}")

    of_matches = re.findall(r'O\d+\s+of\s+(\d+)', content)
    for m_str in of_matches:
        if int(m_str) != TOTAL_LESSONS:
            report("FOOTER", num, f'"O… of {m_str}" — expected of {TOTAL_LESSONS}')
...
print(f"Audited {len(LESSON_FILES)} K-OCP lesson files.")
